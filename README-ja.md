...
2. 変更系のツール呼び出しの前にタスクコントラクト（GOAL / CONSTRAINTS /
   DONE-CRITERIA / CHECK / NON-GOALS / REJECTED-DELIVERABLES /
   RISKIEST-ASSUMPTION）を書く
...
5. 合格基準一式（DONE-CRITERIA、REJECTED-DELIVERABLES、領域別の
   失敗モードチェックリスト）と成果物だけを渡したフレッシュ
   コンテキストのレビュアーで完了を検証する（作業中の経緯は渡さない）。
   徹底監査級の依頼では複数の独立レビュアーによる多数決へ強度を
   引き上げる
...
- OpenAI, [prompt used for "A Proof of the Cycle Double Cover Conjecture"](https://cdn.openai.com/pdf/04d1d1e4-bc75-476a-97cf-49055cd98d31/cdc_prompt.pdf)
  — 2026年7月の CDC 証明主張を生んだオーケストレーションプロンプト。
  v0.6.0 の REJECTED-DELIVERABLES（成功に見える不合格成果物の列挙）、
  領域別失敗モードチェックリストと難易度移転チェック、委譲時の
  検索衛生の出典。「最も厳密に証明できた導出とその正確な残存ギャップを
  報告する」という残存ギャップ明記ルールも、同じプロンプトから得た後続の
  改良。一方で「諦めるな・何時間もかけよ」という無制限の粘りは意図的に
  採用していない。fablish は反復ループに上限を設け、精密な失敗報告を
  妥当な成果として扱う。
- OpenAI, [GPT-5.6 prompting guide](https://developers.openai.com/api/docs/guides/prompt-guidance-gpt-5p6)
  — プロバイダに依存せず通用する補強を探して精査した一般的な
  プロンプティングガイド。委譲は成果で指定する（手順ではなく到達点を
  述べる）、フル検証が高コストなときのコストスケール最小スモークテスト、
  ループ最小化の歯止め（最小の有用ループで解くが、正確性と必須エビデンス
  がループ数に優先する）の出典。reasoning-effort 段階や programmatic tool
  calling といったプロバイダ固有の部分は取り込んでいない。残りの大半は
  既存設計の裏付けにとどまった（疎な進捗更新、敵対的なフレッシュ
  コンテキスト検証、結論先頭の報告）。
